...
An analysis of diamond prices reveals the relationship between the "4Cs of Diamonds" and their value. Known as `cut`, `color`, `clarity`, and `carat`, the 4Cs [(established by the Gemological Institute of America)](https://www.gia.edu/gia-faq-analysis-grading-4cs) are a common set of classifications to determine the rarity and value of a diamond. An investigation into those relationships shows that while the 4Cs are an integral factor in determining the price of a diamond, carat weight is the most significant factor driving price. A carat converts to approximately 0.2 grams. In a study of 1214 diamonds with carat weights ranging from 0.23ct to 7.09ct, weight was found to be most crucial in determining the market value of a diamond. Our analysis determines that for each 1% increase in carat weight results in an increased value within the range of 1.92% to 1.97%. While our study was able to quantify the relationship between weight and price, the implicit value remains in the *eye of the beholder* as it only can with beauty and art. 
...
```{r, fig.cap='Scatter plot of Diamond Price and Carat Weight'}
ggplot(Data, aes(x=carat, y=price/1000)) + 
  geom_point() + 
  labs(x = "Carat Weight", y = "Price of Diamond ('000s $USD)", title = "Relationship between Diamond Price & Carat Weight")+
...
# Boxplot
ggplot(Data, aes(x=clarity, y = price/1000)) + 
  geom_boxplot() + 
...
# Boxplot
ggplot(Data, aes(x=color, y = price/1000)) + 
  geom_boxplot() + 
...
# Boxplot
ggplot(Data, aes(x=cut, y = price/1000)) + 
  geom_boxplot() + 
